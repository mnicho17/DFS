...
def compare_snapshot(conn,import_id,name,folder,date,kind,scores,ownership,username):
    ...
    payload=dict(version=1,name=name,username=_dk_username(username),status='unavailable',reason=reason,
    ...
    if snap:
        ...
        for signature,actual in personal:
            ...
            unique[signature]=dict(roster=signature,projected=sum(values),actual=actual)
...
def snapshot_report(conn,username):
    ...
    for encoded, in conn.execute('SELECT payload FROM result_snapshot_reviews ORDER BY import_id'):
        ...
        if usable:
            ...
            lines.append(f"  Player scoring: {len(errors)} base-player observations; MAE {statistics.mean(map(abs,errors)):.2f}; actual-minus-forecast bias {statistics.mean(errors):+.2f} points.")
            for r in sorted(usable,key=lambda r:-abs(r['actual']-r['projected']))[:5]:
