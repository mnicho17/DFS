...
def summarize_projection_coverage(players):
    season=_slate_season(players);sources=Counter();usage=Counter();positions=Counter();missing=[]
    for p in players:
    ...
    return dict(eligible=len(players),season=season,sources=dict(sources),usage=dict(usage),
                positions=dict(positions),missing_count=len(missing),missing_players=missing[:12])
...
def format_projection_coverage(report):
    ...
    lines.append('- Usage evidence: '+'; '.join(f'{key}: {value}' for key,value in sorted(report['usage'].items()))+'.')
