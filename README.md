...
For a specific NFL Classic contest, open **Settings > Contest-Aware SIM** and save its field size, entry fee, your entry count, and payout tiers. The SIM then replaces the generic payout-shape proxy with that contest's actual prizes, shares covered prizes across tied or duplicated finishes, and displays expected payout, profit, and ROI for each candidate and the selected portfolio. Saved profiles are reusable and local to the current Windows user. Choose **Use Preset Only** at any time to return to the normal preset workflow.
