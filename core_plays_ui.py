...
class CorePlaysDialog(QtWidgets.QDialog):
    def __init__(self,players,kind,parent=None):
        ...
        self.category=QtWidgets.QComboBox();self.category.addItems(['Core candidates','All eligible','All loaded']+list(CATEGORIES))
    ...
    def refresh(self):
        ...
        def include(r):
            ...
            if mode=='Core candidates':return bool(POSITIVE.intersection(r['categories']))
            if mode=='All eligible':return r['eligible']
            if mode=='All loaded':return True
            return mode in r['categories']
        self.visible=[r for r in self.report['rows'] if include(r)]
        ...
        self.table.setSortingEnabled(True);self.table.sortItems(4,QtCore.Qt.DescendingOrder)
    ...
    def copy_report(self):
        QtWidgets.QApplication.clipboard().setText(format_core_report(self.report,self.ordered_rows()))
