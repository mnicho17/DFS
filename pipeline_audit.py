"""Descriptive quarterback counts; never changes candidates or their ranking."""
import math
...
def quarterback_mix(lineups, *, scored=False):
    ...
    for lineup in lineups:
        ...
        positions = [str(p.get('Position') or '').upper().split('/') for p in roster]
        key = str(sum('QB' in p for p in positions)) if positions and all(p != [''] for p in positions) else 'unknown'
...
def format_quarterback_pipeline(stages):
    if not stages:
        return []
    lines = ['', 'Quarterback construction through the build']
    ...
    for stage, data in stages.items():
        ...
        for key, group in sorted(data.get('groups', {}).items()):
            ...
            text = f"{key} QB: {n:,}/{total:,} ({100*n/max(1,total):.1f}%)"
            if group.get('scored'):
