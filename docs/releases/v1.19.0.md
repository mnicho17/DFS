...
### Showdown concentration diagnostics

- Report original build-time Captain locks, selection mode, and retained-lineup count.
- Compare the top individually ranked candidates with the final output by Captain and QB count when independent validation completes.
- Keep all lineup construction, forecasts, simulations, and exposure rules unchanged.

### Offline construction experiment

- Added a salary-verified, frozen-input Showdown comparison for broader construction coverage and QB scoring sensitivity.
- Checks paired outcome draws and opponent identities, saves diagnostic banks, and leaves production scoring and selection unchanged.

### Results construction detail

- Separate Showdown defense and kicker counts, with zero/one/two-defense rows shown explicitly.
- Report the top-1% finish rate within each construction for Classic and Showdown.
- Refresh older construction caches on Analyze Saved Results so missing defense counts are not treated as zero.

### Defense selection audit

- Trace defense counts through the Deep Showdown pipeline, including individual leaders versus selected outputs.
- Reuse existing simulation results; no extra simulation runs, scoring changes or exposure limits.

## Focused Core Plays review
