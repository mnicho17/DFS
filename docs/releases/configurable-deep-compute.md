...
### Per-player usage fallback and build-pool coverage

### Frozen-shortlist repeatability

### Ownership matching, accuracy and leverage

Classic and Showdown now offer bounded feedback matching of explicit slot ownership targets, retaining the legal sampled field with the lowest ownership error across the original plus two adjusted draws. Reports disclose improvement or skipped matching; candidate-generation and unique Classic pools retain their previous path. Ownership & Leverage adds a sortable field/sample/contender/selected exposure comparison, with separate Showdown Captain/FLEX rows and uncalibrated confidence labels. Gaps do not automatically change exposure limits. Results audits measure earliest matched export ownership MAE/bias by contest and slot with explicit coverage and timing limits. New Showdown exports correct FLEX ownership storage; ambiguous legacy values are excluded from accuracy. Guides and screenshot updated. This changes field sampling, so previously saved ranking banks require a new Deep build under the updated code.

Deep Classic and Showdown automatically preserve their validated shortlist before selection. Settings > Ranking Repeatability compares the same bank across 2–10 fresh scenario/opponent batches, with per-lineup mean/range finish rates, rank ranges and top-50/150 frequency. Text, CSV and JSON reports save automatically; Copy Report is available. Partial batches are excluded, code-version changes are rejected, Captain identities survive serialization, and original output rankings remain unchanged. Old snapshots alone cannot recover an earlier shortlist. This measures sampling variation, not predictive accuracy. The user guide includes controls and backup locations.

The Classic role pool now ranks verified rotation slots ahead of unknown depth, correcting a fallback that could displace known WR2/WR3 and RB2 players with unranked reserves. Shared NFL eligibility excludes explicitly missing forecasts and reports locked missing forecasts as actionable errors. Valid supplied zeros, rookie workload estimates and legacy inputs remain supported. This changes candidate eligibility in both NFL formats; replaying old inputs under new code can therefore produce different lineups.

Partial current-season statistics no longer block prior-season usage for every other player. Fresh enrichment matches each player against current records first, preserving observed zeros, then prior-season records. Usage year/source is retained in player inputs and exports. Classic and Deep Showdown reports count projection sources, current/prior/missing usage, and truly missing forecasts in the eligible pool instead of warning about every loaded estimate. Snapshot replay remains unchanged; reload salaries to use the new fallback.
