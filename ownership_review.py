...
def load_review(path, bank_folder):
    path = Path(path)
    ...
    return dict(report=report, rows=rows, kind=payload['kind'], compatible=report.get('current_model') == model_version())

def comparison_value(window, lineup, kind):
    review = getattr(window, '_' + kind + '_ownership_review', None)
...
def tooltip(review):
    r = review['report']
    return (f"Saved ownership comparison: bank {r['bank_id'][:12]}; {r['completed_batches']} batches, {r['scenarios']:,} scenarios/profile.\n"
            ...
            'Rank min/max span all profiles and completed batches within the saved candidate bank. These are tested model outcomes, not confidence bounds or guaranteed worst cases. Sorting does not optimize a portfolio.')
