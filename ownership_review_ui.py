...
from ownership_review import COLUMNS, KEYS, load_review, tooltip

class ReviewDialog(QtWidgets.QDialog):
    def __init__(self, parent, review):
        super().__init__(parent); self.setWindowTitle('Saved Ownership Comparison'); self.resize(1150,650)
...
def open_review(parent, kind=None):
    history=Path(build_history_path()).parent
    path,_=QtWidgets.QFileDialog.getOpenFileName(parent,'Choose saved ownership comparison',str(history/'ownership-checks'),'Comparison reports (*.json)')
    if not path:return
    try:
        review=load_review(path,history/'ranking-banks')
    ...
    if kind:
        ...
        setattr(parent,'_'+kind+'_ownership_review',review)
        choice=getattr(parent,'_'+kind+'_sort',None)
        if choice and choice[2] in COLUMNS:setattr(parent,'_'+kind+'_sort',None)
        ...
        matched=sum(comparison_value(parent,lu,kind) is not None for lu in current)
        parent.status.showMessage(f'Ownership comparison: {matched}/{len(current)} exact player-input matches. Unmatched rows show a dash.',10000)
